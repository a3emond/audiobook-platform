...
export async function handleReplaceCoverJob(
  ...
  try {
    ...
    if (bypassChapterOverride && (book.overrides?.chapters ?? false)) {
      logger.warn("Chapter override bypass enabled during replace-cover", {
        bookId: payload.bookId,
        source:
          payload.forceChapterTimingRepair === true
            ? "payload"
    ...
    if (!chapterOverrideLocked && chaptersForRemux.length > 0) {
      const durationMs = Math.round(
        (await ffmpeg.probeFile(book.filePath)).duration * 1000,
      );
      const repaired = repairChapterTimingScale(chaptersForRemux, durationMs);

      if (repaired) {
        chaptersForRemux = repaired.chapters;
        repairedChapters = repaired.chapters;
        logger.warn("Replace cover auto-repaired chapter timing scale", {
          bookId: payload.bookId,
          count: chaptersForRemux.length,
