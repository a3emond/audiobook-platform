...
import { JobModel, type JobDocument, type JobType } from "../queue/job.types.js";
...
async function hasActiveBookJob(type: JobType, bookId: string): Promise<boolean> {
...
async function enqueueBookJob(type: JobType, payload: Record<string, unknown>, priority: number): Promise<void> {
	...
	try {
		...
		for (const book of books) {
			...
			if (!book.filePath) {
				errors += 1;
				drifted += 1;
				logger.warn("Book missing filePath during rescan", { bookId: String(book._id) });
			...
			try {
				...
				const checksumMismatch = Boolean(book.checksum && book.checksum !== checksum);
				const durationMismatch = typeof book.duration === "number" && Math.round(book.duration) !== duration;
				const dirtyStatus = (book.fileSync?.status ?? "in_sync") !== "in_sync";
				const needsSanitize = isMp3Source(book);
				const needsWriteMetadata = !needsSanitize && (dirtyStatus || checksumMismatch || durationMismatch);
				const now = new Date();
				...
				if (needsSanitize || needsWriteMetadata) {
						...
						{ _id: book._id },
						{
							$set: {
								"fileSync.status": needsSanitize
									? (book.fileSync?.status === "error" ? "error" : "dirty")
									: "dirty",
					...
					if (needsSanitize) {
						...
						if (await hasActiveBookJob("SANITIZE_MP3_TO_M4B", bookId)) {
							...
							logger.debug("Skipped sanitize enqueue; active job already exists", { bookId });
						} else {
						...
						if (await hasActiveBookJob("WRITE_METADATA", bookId)) {
							skippedExistingRemediation += 1;
							logger.debug("Skipped metadata remediation enqueue; active job already exists", { bookId });
						} else {
						...
						logger.warn("Detected parity drift without direct remediation path", {
							bookId: String(book._id),
							processingState: book.processingState ?? "ready",
							filePath: book.filePath,
							checksumMismatch,
